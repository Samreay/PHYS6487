...
from smithcorr import SmithCorr
import pickle
...
def getRChi(om, ol, z, h, quality=100):
    prefactor = cks / (h * 100)
    zs = np.linspace(0, z, quality)
    integrand = 1.0 / vgetEz(om, ol, zs)
    return prefactor * simps(integrand, zs)
    
def getDa(om, ol, z, h):
    return getRChi(om, ol, z, h) / (1.0 + z)
    
def getHz(om, ol, z, h):
    return np.sqrt(getEz(om, ol, z)) * h
...
class SlowGenerator(object):
    def __init__(self, debug=False, generate=False, omch2Range=[0.05,0.25], zs=[0.44, 0.6, 0.73]):
        self.saveFolder = "C:\\slowSaves4"
        self.prefix = "cambAndSmithChunk.dat"
    ...
    def loadChunck(self):
        if self.debug:
            print("Loading pickle from " + self.saveFolder)
        try:
            with open(self.saveFolder + os.sep + self.prefix) as f:
