...
\end{align}
{\red Check with Tam the planned way of using $\alpha$ and $\epsilon$ and getting the derivatives is sufficient, at the moment have simply done the naive way above} These models were compared to all 600 WizCOLA realisations, such that the WizCOLA data points were calculated from the mean of all realisations, and the covariance reduced by a factor of $\sqrt{600}$. Data was fit in the range $40<s<180$, and a comparison of the effects of dataset truncation can be found in Appendix \ref{app:truncation}.

The fitting results for the $z=0.6$ redshift bin are $\Omega_c h^2 = 0.114^{+0.007}_{-0.006}$, $\log(k_*) = -1.99^{+0.30}_{-0.24}$ and $\alpha = 1.003^{+0.020}_{-0.016}$, and the likelihood surfaces and marginalised distributions are shown in Figure \ref{fig:wizmp}. The best fitting and mean models have been reproduced in Figure \ref{fig:wizmodel}. The desired recovery of WizCOLA simulation conditions of $\alpha = 1.0$ and $\Omega_c h^2 = 0.113$ are within the uncertainty of the results found. The minimum $\chi^2$ of the result is $8.2$, and with 14 degrees of freedom (21 data points with 7 free parameters) this indicates an overly good fit to the data, as might be expected given the combination of all 600 realisations. Finally, the maximum likelihood $\log(k_*)$ of $-1.99$ corresponds to a $\sigma_v = 5.18$, which is in the magnitude predicted by the theory given in equation \eqref{eq:sigmav}.
...
%\begin{wrapfigure}{r}{0.5\textwidth}
  \begin{center}
    \includegraphics[width=\textwidth]{images/wizcola_quad_z1_40_180.pdf}
  \end{center}
  \caption{Model fitting using data from the WizCOLA $z=0.6$ bin, where we are fitting to the multipole expansion. The dashed line represents the desired parameter recovery of $\Omega_c h^2 = 0.113$. Increasing the range of data points included in the matching served to draw the recovered $\Omega_c h^2$ further away from the desired recovery value. We can also see that moving from varying $\sigma_v$ as has been done in \citet{BlakeDavis2011} to varying $\log(k_*)$ we span a far larger parameter space, but are able to actually recover constraints on $k_*$.}
  \label{fig:wizmp}
%\end{wrapfigure}
\end{figure}
...
%\begin{wrapfigure}{r}{0.5\textwidth}
  \begin{center}
    \includegraphics[width=\textwidth]{images/wizcola_quad_z1_40_180model.pdf}
  \end{center}
...
{\red Note this section is out of data, I ran this fit using fixed $f_b$ and not fixed $\Omega_b h^2$}.
