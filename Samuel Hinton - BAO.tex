...
\end{align}
If we wish to find the distance between two objects at time $t_0$, one can simply rotate the coordinate system such that $d\Omega$ vanishes and then integrate, giving that $D = R(t_0) \chi \equiv R_0 \chi$. Thus $\chi$ represents a distance independent of the the scale of the universe, which is denoted the comoving distance \citep{CarrollOstlie2006}.  As such, $R_0 \chi$ represents the distance $D$ between two objects if measured in the present day.\\
